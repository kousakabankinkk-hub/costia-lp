...
オプションの価格は `src/lib/stripe.ts` の `OPTION_PLANS`、領収書の読み取り上限は
`src/lib/receipt-parse-limit.ts` が正です(LP の「オプション」「料金」「よくある質問」に写してある)。

「話して日報」(音声で日報の下書きを作る機能)は、プライバシーポリシーの記載が
弁護士確認待ちのため、2026-09-21 時点では LP に載せていない。
