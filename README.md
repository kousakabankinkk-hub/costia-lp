...
| ファイル名 | 置く場所 | 写すもの | 注意 |
|---|---|---|---|
| `assets/screen-project-link.png` | オプション節(案件連携) | Telegram の現場トピックに写真を送り 👍 が付いた画面と、Drive の現場フォルダに写真/書類/連絡メモが並んだ画面を横に並べる(または片方) | Telegram の**個人名・アイコンを写さない**(デモ用アカウントで撮るか加工で消す)。現場名はデモデータのもの |
